...
async function main() {
  ...
  await upsertEngineeringCase({
    ...
    summaryZh: '在為台灣、美國、加拿大、墨西哥、馬來西亞、菲律賓、土耳其等多個市場維護同一套電商平台的過程中，處理各市場在地址規則、結帳流程、物流與金流整合上的差異。',
    summaryEn: 'While maintaining a single e-commerce platform across markets including Taiwan, the United States, Canada, Mexico, Malaysia, the Philippines, and Turkey, handled differences in address rules, checkout flows, logistics, and payment integrations between markets.',
    problemZh: '同一套結帳與訂單邏輯無法直接套用到所有市場，不同國家在地址欄位格式、必填資訊、配送方式與可用金流服務上都存在差異，若以單一邏輯處理容易在特定市場產生錯誤。',
    problemEn: 'A single checkout and order logic cannot be applied uniformly across all markets; countries differ in address field formats, required information, shipping methods, and available payment options, and a one-size-fits-all approach easily produces errors in specific markets.',
    contextZh: '平台需要在維持核心商業邏輯一致的前提下，容納各市場各自的規則差異，同時避免程式碼因為市場別分支而變得難以維護。',
    contextEn: 'The platform needed to accommodate per-market rule differences while keeping the core business logic consistent, without letting market-specific branching make the codebase unmaintainable.',
    investigationZh: '比對多個市場的結帳流程後發現，差異主要集中在地址欄位驗證規則、可用的物流選項與金流服務組合這幾個面向，而非整體訂單流程本身。',
    investigationEn: 'Comparing checkout flows across markets revealed that differences were concentrated in address field validation rules, available shipping options, and the combination of supported payment services — not in the overall order flow itself.',
    solutionZh: '將市場別差異限縮在地址驗證規則與可用服務清單的設定層級，核心訂單狀態機與結帳流程維持共用，並依市場動態載入對應的地址欄位與可選物流 / 金流選項。',
    solutionEn: 'Confined market-specific differences to configuration-level address validation rules and available service lists, keeping the core order state machine and checkout flow shared, while dynamically loading the appropriate address fields and shipping/payment options per market.',
    validationZh: '針對每個市場分別以該國實際地址格式與可用金流組合進行結帳流程測試，確認地址驗證、物流選項與金流串接皆能正確運作。',
    validationEn: 'Tested the checkout flow separately for each market using that country\'s actual address formats and available payment combinations, confirming address validation, shipping options, and payment integration all worked correctly.',
    resultZh: '同一套平台得以穩定支援多個市場的結帳流程，新增或調整市場規則時只需異動設定層級，不需重構核心訂單邏輯。',
    resultEn: 'The same platform reliably supports checkout across multiple markets, and adding or adjusting market rules only requires configuration-level changes rather than refactoring the core order logic.',
    architecture: JSON.stringify([
      { labelZh: '地址規則', labelEn: 'Address Rules' },
      { labelZh: '結帳流程差異', labelEn: 'Checkout Differences' },
      { labelZh: '物流', labelEn: 'Logistics' },
      { labelZh: '金流', labelEn: 'Payment' },
      { labelZh: '在地整合', labelEn: 'Local Integrations' },
    ]),
